...
import { Acv, barPositions, hInput, hwOverLw, lwInput } from "../wall";
...
/** √f'c in psi, namespaced per check. */
export function sqrtFcNode(w: WallInput, ns: string): Traced {
  const fc = fcInput(w.concrete);
  const value = sqrtFcPsi(w.concrete.fc);
  return derive({
    id: `${ns}.sqrt_fc`,
    symbol: "√f'_c",
    ...
    unit: "psi",
    formula: "\\sqrt{f'_c}",
    substitution: `\\sqrt{${fmtTex(ksiToPsi(w.concrete.fc))}} = ${fmtTex(value, { dp: 1 })}\\ \\text{psi}^{0.5}`,
    inputs: [fc],
  });
}

/** n·λ√f'c·A_cv in kip — the shear yardstick both 18.10.2.1 and 18.10.2.2 use. */
function shearYardstick(
  ...
  const c = constant(
    `${ns}.${key}_coeff`,
    String(coeff),
    "in-lb coefficient on λ√f'c·A_cv",
    coeff,
    "1",
    aci(ref),
  );
  ...
  return derive({
    id: `${ns}.${key}`,
    symbol: `${coeff}λ√f'_c·A_cv`,
    label: "shear threshold",
    value,
    unit: "kip",
    formula: `${coeff}\\,\\lambda\\sqrt{f'_c}\\,A_{cv}`,
    substitution:
      `${coeff} \\times ${fmtTex(w.concrete.lambda, { dp: 2 })} \\times ${fmtTex(sqrt.value, { dp: 1 })} \\times ` +
      `${fmtTex(acv.value)} = ${fmtTex(value)}\\ \\text{kip}`,
    ref: aci(ref),
    inputs: [c, lambda, sqrt, acv],
    note: "psi × in² → lb, reported in kip",
  });
}
...
export function checkSeismicWebReinforcement(w: WallInput, demand: Demands): CheckResult {
  ...
  const lowShearLimit = shearYardstick(w, NS, "limit_1", 1, "18.10.2.1", sqrt, acv);
  const lowShear = Math.abs(demand.Vu) <= lowShearLimit.value + TOL;
  const lowShearTrigger = derive<boolean>({
    ...
    value: lowShear,
    unit: "1",
    formula: "V_u \\le \\lambda\\sqrt{f'_c}\\,A_{cv}",
    substitution: `${fmtTex(Math.abs(demand.Vu))} \\le ${fmtTex(lowShearLimit.value)} \\Rightarrow \\text{${lowShear}}`,
    ref: aci("18.10.2.1"),
    inputs: [Vu, lowShearLimit],
    note: lowShear
      ? "18.10.2.1 permits the Table 11.6.1 minimums (0.0012/0.0020 for Grade 60 bars ≤ No. 5)"
      : "the 0.0025 minimums of 18.10.2.1 govern",
  });

  // Table 11.6.1 relaxation, Grade 60 deformed bars: ρ_ℓ 0.0012 / ρ_t 0.0020 for
  // No. 5 and smaller, 0.0015 / 0.0025 above. Bar sizes are read per layer.
  const relaxed = (bar: string): { l: number; t: number } =>
    Number(bar) > 5 || w.grade.fy < 60 ? { l: 0.0015, t: 0.0025 } : { l: 0.0012, t: 0.002 };
  ...
  const reqNode = (key: "l" | "t", layer: { bar: string }): Traced => {
    ...
    return derive({
      ...
      value,
      unit: "1",
      formula: "\\rho_{min} = 0.0025 \\ \\text{unless}\\ V_u \\le \\lambda\\sqrt{f'_c}A_{cv}",
  ...
  const sProv = input(
    ...
    Math.max(w.vertical.spacing, w.horizontal.spacing),
    "in",
  );
  const utilS = derive({
    id: `${NS}.util_s`,
  ...
  const twoCurtainLimit = shearYardstick(w, NS, "limit_2", 2, "18.10.2.2", sqrt, acv);
  const byShear = Math.abs(demand.Vu) > twoCurtainLimit.value + TOL;
  ...
  const curtainsReq = derive({
    ...
    unit: "1",
    formula: "n_{req} = 2 \\ \\text{if}\\ V_u > 2\\lambda\\sqrt{f'_c}A_{cv}\\ \\text{or}\\ h_w/\\ell_w \\ge 2.0",
    substitution:
      `V_u = ${fmtTex(Math.abs(demand.Vu))} > ${fmtTex(twoCurtainLimit.value)} \\Rightarrow \\text{${byShear}};\\ ` +
 ...
 * over one wall thickness, must be at least 6√f'c/f_y (psi). The provided ratio
 * is summed from the resolved bar stations with x < 0.15ℓ_w.
 *
...
function endZoneRatio(w: WallInput, ratio: Traced, sqrt: Traced): EndZoneNodes {
  ...
  const lengthValue = 0.15 * w.geometry.lw;
  const length = derive({
    ...
    unit: "in",
    formula: "0.15\\,\\ell_w",
    substitution: `0.15 \\times ${fmtTex(w.geometry.lw)} = ${fmtTex(lengthValue, { dp: 1 })}\\ \\text{in.}`,
    ref: aci("18.10.2.4"),
    inputs: [fraction, lw],
  });
  ...
  const As = input(
    ...
    AsValue,
    "in2",
    `${stations.length} bar station(s) at x < ${fmtTex(lengthValue, { dp: 1 })} in. from the wall end`,
  );

  const areaValue = lengthValue * w.geometry.h;
  const area = derive({
    id: `${NS}.end_zone_area`,
    symbol: "A_end",
    ...
    substitution: `${fmtTex(lengthValue, { dp: 1 })} \\times ${fmtTex(w.geometry.h, { dp: 1 })} = ${fmtTex(areaValue)}\\ \\text{in}^2`,
    ref: aci("18.10.2.4"),
    inputs: [length, h],
  });
  ...
  const coeff = constant(
    `${NS}.end_zone_coeff`,
    "6",
    "in-lb coefficient of the 18.10.2.4 end-zone minimum",
    6,
    "1",
    aci("18.10.2.4"),
    "SI form 0.5√f'c/f_y (MPa)",
  );
  const fyPsi = ksiToPsi(w.grade.fy);
  const fy = input(`${NS}.fy`, "f_y", "specified yield strength of reinforcement", fyPsi, "psi");
  const reqValue = (6 * sqrt.value) / fyPsi;
  const req = derive({
    ...
    value: reqValue,
    unit: "1",
    formula: "\\rho_{end,min} = \\dfrac{6\\sqrt{f'_c}}{f_y}",
    substitution: `\\dfrac{6 \\times ${fmtTex(sqrt.value, { dp: 1 })}}{${fmtTex(fyPsi)}} = ${fmtTex(reqValue, { dp: 5 })}`,
    ref: aci("18.10.2.4"),
