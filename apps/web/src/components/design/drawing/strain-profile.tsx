...
import { dim, lenDim, strain } from "./format";
...
export function StrainProfile({
    ...
    return (
      ...
      <div className="flex min-h-40 flex-col items-center justify-center gap-1 rounded-sm border border-dashed border-border p-6 text-center">
        ...
        <p className="max-w-64 font-mono text-xs2 text-muted-foreground/70">
          the section cannot equilibrate Pu = {dim(U.force(Pu), 0)} {U.forceUnit} — there is no
          neutral-axis depth to draw
        </p>
      </div>
    );
  }
  ...
  const Y0 = T + GAP;
  // `slice.c` is already in the wall's reporting unit (in | mm), but every
  // coordinate here is scaled off `lw` in canonical inches, so the geometry
  // works from `cIn` and only the labels use the reported number.
  const cIn = U.toIn(slice.c);
  const a = Math.min(beta1(input.concrete, U.scheme).value * cIn, lw);

  const emax = Math.max(EPS_CU, slice.epsT, 1e-9);
  const yTop = Y0 - (EPS_CU / emax) * AMPLITUDE;
  const yBot = Y0 + (slice.epsT / emax) * AMPLITUDE;
  const cX = Math.min(cIn, lw) * s;
  const tX = xt * s;
  ...
  return (
    <Drawing
      ...
      fontSize={FONT}
      title={`strain profile — neutral axis at c = ${lenDim(U, cIn, 1)} ${U.lengthUnit}, εt = ${strain(slice.epsT)}, φ = ${dim(slice.phi, 2)}`}
    ...
    >
      ...
      <DimLine x1={0} y1={0} x2={a * s} y2={0} offset={-24} label={`a = ${lenDim(U, a, 1)}`} />
      <DimLine x1={0} y1={0} x2={cX} y2={0} offset={-48} label={`c = ${lenDim(U, cIn, 1)}`} />
      ...
      <Note x={WALL_W} y={-46} anchor="end" size={9}>
        {/* φPn and φMn come out of the slice already in the wall's system; Pu is
            a demand, which is stored canonical, so only that one converts. */}
        {traced !== undefined
          ? `φPn ${dim(slice.phi * slice.Pn, 0)} ${U.forceUnit} · φMn ${dim(slice.phiMn, 0)} ${U.momentUnit}`
          : `Pu ${dim(U.force(Pu), 0)} ${U.forceUnit} · φMn ${dim(slice.phiMn, 0)} ${U.momentUnit}`}
      </Note>
    </Drawing>
  );
}
